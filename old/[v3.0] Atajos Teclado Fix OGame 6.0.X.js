...
(function ()
{
	...
	function addID(){		
		$("#speedLinks a").each(function(index){ //Recorremos el div y por cada elemento que tenga etiqueta <a></a>, se agregará un id, en este caso será "speed_button_0" ... "speed_button_9"							
	...
	$ (document).keydown (function (e)
	{
		if (($ ("div").is ("#anti_options_window")) ||
		    (($ ("div").is ("#TB_window") ||
		      ...
		      $ ("body").is ("#search") ||
		      $ ("body").is ("#showmessage") ||
					$ ("textarea").is (".new_msg_textarea") || /*Agregada por Ivan para que los accesos directos no funcionen en los campos de texto en la página de chat, comunicación de la alianza*/
					$ ("textarea").is (".chat_box_textarea") || /*Agregada por Ivan para que los accesos directos no funcionen en las pequeñas ventanas de chat (cuando se tiene la barra de chat activa)*/
		...
		switch (e.keyCode)
		{
			...
			case 27:	// Esc
				...
				else if ($ ("span.ui-icon-closethick").length > 0)
				{
					...
					simulateMouseClick ( $(".icon_close") ); /*Cierra ventanas de chat (cuando barra de chat está activada)*/
				}
				break;
			case 33:	// PgUp
				if (e.ctrlKey || e.altKey || e.metaKey)
			...
			case 49:	// "1"
				...
				if (e.shiftKey)
				{
					...
					if( theHref.indexOf("game/index.php?page=messages&tab=1&ajax=1") ) { /* Communication | Comunicación */
						simulateMouseClick( $("#ui-id-53") ); /* Messages | Comunicados */
					}
				} else { /*Lines added by Ivan |A partir de aquí, lineas agregadas por Ivan*/
					if ( theHref.indexOf("/game/index.php?page=messages") > -1 ){
			...
			case 50:	// "2"
				...
				if (e.shiftKey)
				{
					...
					if( theHref.indexOf("game/index.php?page=messages&tab=1&ajax=1") ) { /* Communication | Comunicación */
						simulateMouseClick( $("#ui-id-55") ); /* Information | Información */
					}
				} else { /*Lines added by Ivan |A partir de aquí, lineas agregadas por Ivan*/
					if ( theHref.indexOf("/game/index.php?page=messages") > -1 ){
